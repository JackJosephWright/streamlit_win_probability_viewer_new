...
from load_id_to_wr_mappings import load_id_to_wr_mappings
from load_id_to_name_mapping import load_id_to_name_mapping

def transform_row_to_turns(row, id_to_wr_mapping = None, max_cards=20, display = False):
    """
    ...
    if id_to_wr_mapping is None:
        id_to_wr_mapping = load_id_to_wr_mappings()
    if display:
    ...
    for prefix in ['user_hand', 'oppo_hand', 'user_creatures', 'oppo_creatures', 'user_non_creatures', 'oppo_non_creatures']:
        if display:
            turn_df = apply_winrate_mapping(turn_df, id_to_name_mapping, prefix, max_cards=max_cards)
        else:
